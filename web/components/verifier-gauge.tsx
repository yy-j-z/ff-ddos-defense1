...
export function VerifierGauge({ result }: { result: VerificationResult | null }) {
  if (!result) {
    return <p className="text-xs text-slate-500">等待验证结果</p>;
  }
  const tone = scoreTone(result.score);
  const blockedRatio = result.totalRequests
    ? result.blockedRequests / result.totalRequests
    : 0;
    ...
    <div className="space-y-3">
      <div>
        <div className="flex items-baseline justify-between">
          <span className="text-xs text-slate-400">绕过得分</span>
        ...
        <div className="mt-1.5 h-1 w-full overflow-hidden rounded-full bg-[#1e293b]">
          <div
      ...
      <dl className="grid grid-cols-3 gap-x-4 gap-y-2.5 text-xs">
        ...
        <Metric
          ...
          tone={result.defenderTriggered ? 'text-amber-400' : 'text-emerald-400'}
        />
        <Metric label="总请求" value={result.totalRequests.toLocaleString()} />
        <Metric label="被拦截" value={`${(blockedRatio * 100).toFixed(0)}%`} />
        <Metric
          label="触发耗时"
          value={result.defenderLatencyMs ? `${result.defenderLatencyMs}ms` : '—'}
        />
      </dl>

      <div className="flex flex-wrap items-center gap-x-4 gap-y-2 text-xs">
        <div className="flex items-center gap-2">
          <span className="text-slate-400">业务影响</span>
        ...
        {result.defenderRulesHit.length > 0 && (
          <div className="flex flex-wrap items-center gap-1.5">
            ...
            {result.defenderRulesHit.map((r) => (
              <span
                key={r}
                className="rounded-sm bg-[#1e293b] px-1.5 py-0.5 font-mono text-[10px] text-slate-400"
              >
                {r}
              </span>
            ))}
